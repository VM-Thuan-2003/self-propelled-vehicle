...
#####---Load Model---######
state_dict = torch.load('test1_model.pth', map_location='cpu')
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            # preImage_11 = preImage(image)
            # cv2.imshow("preImage_11",preImage_11)
            #image = image*0.6
            image = image.astype('uint8')
            imageCrop = image[195:, :]
            ...
            if torch.amax(out2) >= 0.5:
                ...
                local_set = 60
                turncheck[0] = cv2img[local_set,100][0]
                turncheck[1] = cv2img[local_set,240][0]
                turncheck[2] = cv2img[local_set,380][0]
                #print(turncheck)
            ###############################################################################################
            frame += 1
            if frame % 1 == 0:
            ...
            position = np.sum(weight*y_cordinate) # this is center of segmentation
            #print(np.sum(a))
            error = position - 240
            ...
            if T_right == 1 and turncheck[0] == 0 and turncheck[1] == 0 and turncheck[2] == 0:
                ...
                if time.time() - pre_time_right < tmr_quay:
                    error = err_sign
                    flag_right = 1
                else:
                    pre_time_right = time.time()
            ...
            if T_left == 1 and  turncheck[0] == 0 and turncheck[1] == 0 and turncheck[2] == 0:
                ...
                if time.time() - pre_time_left < tmr_quay:
                    error = -err_sign
                    flag_left = 1
                else:
                    T_left = 0
            ...
            print(T_right, T_left, out_sign,flag_right,flag_left,error, pre_err, angle,turncheck[0],turncheck[1],turncheck[2],flag_left,flag_right,T_left,T_right)

            speed = 100
            p = .39
            i = 0
            d = 0.098
            angle =  PID(error*1.1,p,i,d) #0.42 - 0 - 0.09
            ###################################################################################
